...
### Fixed
- **Help printed from parent parser**: invoking an app without a subcommand now prints help from the parent parser, correctly listing all available subcommands instead of showing a subparser's narrower help.
...
### Added
- **`exec` built-in command**: batch-processes a JSONL input file, dispatching each record as a subcommand invocation and collecting results.

## [0.1.15] - 2026-05-11

### Changed
- **`model_dump(exclude_none=True)`**: Pydantic models serialised via `_to_dict` now omit `None`-valued fields from JSON/JSONL/TSV output, keeping agent payloads compact.
- **Internal refactor**: `result`, `external_data`, `_normalise`, and `_default_json` now route through the `_to_dict` helper consistently, eliminating duplicated `isinstance`/`hasattr` branches.

## [0.1.14] - 2026-05-11

### Added
- **`--help-full` flag**: shows all flags including hidden ones (e.g. `--format`, format shorthands) for power users and agent inspection.
- **`global_flags` in schema**: `get_schema()` now includes a `global_flags` array listing all injected global flags with their descriptions, choices, and hidden status.
- **Format shorthand flags**: `--json`, `--csv`, and `--tsv` are now individual shorthand flags (each sets `--output` to the corresponding format), replacing the previous boolean `--json` flag.

### Fixed
- **`IO` import**: added missing `IO` import from `typing` in `_app.py`.

## [0.1.13] - 2026-05-10

### Added
- **`--output`/`-o` flag**: select output format (`json`, `jsonl`, `tsv`, `plain`, `table`) per-invocation without environment variables.
- **`--json` shorthand flag**: convenience alias for `--output json`, enabling quick machine-readable output.
- TSV and plain-text formatters alongside the existing JSON/JSONL/table renderers.
